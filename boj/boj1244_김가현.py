...
for i in range(students) : # 학생 수만큼 작업 반복. 성별에 따라 솔루션 다름
    if information[i][0] == 1 : # 남학생이라면
        ...
        for j in range(1, len(switches)+1) : # 스위치 수만큼 반복
            if j % number == 0 : # 나눈 값이 곧 배수를 뜻함. 스위치의 위치에 따른 배수라면
                if switches[j-1] == 1 : # 0을 1로 바꾸고 1을 0으로 바꿈
                    switches[j-1] = 0
                elif switches[j-1] == 0 :
                    switches[j-1] = 1
    ...
    if information[i][0] == 2 : # 여학생이라면
        ...
        while True :
            if number-x-1 < 0 or number+x-1 > N :
                break
            if switches[number-x-1] == switches[number+x-1] : # 스위치 값이 같다면
